...
if st.session_state['selected_drugs']:
    ...
    if len(input_ids) >= 2:
        ...
        if all(s is not None for s in smiles_list):
            ...
            for (id1, sm1), (id2, sm2) in combinations(zip(input_ids, smiles_list), 2):
                ...
                if all(x is not None for x in [fp1, fp2, emb1, emb2]):
                    ...
                    st.write(f"💡 **{id1} + {id2}** → Likelihood: **{likelihood:.2%}**")
                else:
                    st.warning(f"⚠️ Could not generate all features for {id1} + {id2}")
        else:
